...
tError funkce()
{
    ...
    if(token.stav != S_KLIC) //pokud nedostanu klicove slovo
        return  ESYN;
    if (!(strcmp(token.data), "declare")) // simple function
    {
        ...
        if(token.stav == S_KLUC) // musi nasledovat as coz je klicova slova
        {
            ...
            if ((token.stav != S_INT) || (token.stav != S_DOUBLE) || (token.stav != S_STRING))// test navratove hodnoty
                return ESYN;
             else {
    ...
    if(!(strcmp(token.data, "scope"))) //pokud mame "scope"
    {
        ...
        if(token.stav == S_KLIC) //klicove slovo
        {
    ...
    else //jinak jina funkce
    {
        ...
        if (token.stav != S_KLUC) return ESYN;
...
tError list()
{
    ...
    switch (token.stav)
    {
    case S_IDENT: //<list>	->  ID <prirazeni> "eol" <list>
        ...
        if((tmpNode != NULL) && (tmpNode->data.varFc == true))
        {
            ...
            if(token.stav != S_EOL) //musim mit eol
                return ESYN;

            break;
        }
        else if((!(strcmp(token.data, "Chr"))) || (!(strcmp(token.data, "substr"))) || (!(strcmp(token.data, "length"))) || (!(strcmp(token.data, "Asc"))))
        {
            error = vest();
            if(error != EOK)
                return error;
    ...
    if((token.stav == S_STR) || (token.stav == S_INT) || (token.stav == S_DOUBLE) || (token.stav == S_EXPO)) //musi byt STRING nebo CISLO
    {
        ...
        */
        ...
        if(error != EOK)
            return error;
            */
        if(token.stav != S_EOL) //musi byt "eol"
            return ESYN;
...
tError vest()
{
    ...
    else if(token.stav != S_EOL) //musi byt "EOL"
        return ESYN;
...
tError varDeclar()
{
    ...
    if(token.stav != S_IDENT) //ID... za dim musi byt ID
        return ESYN;
    ...
    if(error != EOK)
        return error;
    if ((token.stav != S_INT) || (token.stav != S_DOUBLE) || (token.stav != S_STR))
        return ESYN;
    if ((token.stav == S_DOUBLE) || (token.stav == S_INT))
    {
        getToken();
        if(error != EOK)
        return error;
...
tError litExpr()
{
    ...
    switch (token.stav)
    {
    case S_KLUC: //<litexpr> -> <input>
        if(!(strcmp(token.data, "input")))
        {
            error = input();
            break;
        }
